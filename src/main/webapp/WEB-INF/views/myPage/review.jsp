...
<head>
<meta charset="UTF-8">
<title>arTchive / 마이페이지 - 전시 리뷰(게시글)</title>
<link rel="stylesheet" type="text/css"
	href="<c:url value='/tools/reset.css'/>" />
<link rel="stylesheet" type="text/css"
	href="<c:url value='/css/common.css'/>" />
<link rel="stylesheet" type="text/css"
	href="<c:url value='/css/myPage/review.css'/>" />
<script src="https://kit.fontawesome.com/50d21a2bed.js"
	crossorigin="anonymous"></script>
<script src="<c:url value='/tools/jquery-3.6.0.min.js'/>"></script>
<script src="<c:url value='/js/myPage/review.js'/>"></script>
</head>
<body>
	<div class="wrapper">
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach items="${reviewList}" var="reviewList" varStatus="status">
												<tr>
													<td><a
														href="<c:url value='/review/reviewNote/${reviewList.reviewNoteId}'/>">${reviewList.reviewNoteTitle}</a></td>
													<td><fmt:formatDate value="${reviewList.reviewNoteCreatedDate}"
															pattern="yyyy-MM-dd" /></td>
													<td><input type="button" id="submitBtn"
														name="submitBtn" class="white-btn"
														onclick="deleteReviewNote(${reviewList.reviewNoteId})" value="삭제"></td>
												</tr>
											</c:forEach>
										</c:otherwise>
